...
import { ADMIN_EMAIL } from '../config/firebase.js';
import { STATE } from '../core/state.js';
import {
  countUserChars,
  loadChars,
  loadCollection,
  loadCollectionOrdered,
  getDocData,
} from '../data/firestore.js';
import { renderCharSheet } from './characters.js';
import { getDefaultBastion } from './bastion.js';
import { getDefaultTutorial } from './tutorial.js';
import {
  getInfoStats,
  getInfoEquipements,
  getInfoCombat,
...
const PAGES = {
  async dashboard() {
    ...
    const charCount = await countUserChars(STATE.isAdmin ? null : STATE.user?.uid || null);
  ...
  async characters() {
    const uid = STATE.isAdmin ? null : STATE.user?.uid || null;
  ...
  async shop() {
    ...
    let html = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">🛒 Boutique</span></div>
    ...
    if (items.length === 0) {
      ...
      html += `<div class="tabs" style="margin-bottom:1.5rem" id="shop-cats">
  ...
  async world() {
    ...
    const content = document.getElementById('main-content');
    let html = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">📖 Le Monde</span></div>
    ...
    const sections = worldDoc?.sections || [{ title: 'Introduction', content: 'Les informations sur le monde seront ajoutées ici par le Maître de Jeu.' }];
    sections.forEach((s) => {
  ...
  async map() {
    const mapDoc = await getDocData('world', 'map');
    const content = document.getElementById('main-content');
    content.innerHTML = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">🗺️ Carte de la Région</span></div>
      <div class="page-subtitle">Explorer le monde</div>
    </div>
    ${STATE.isAdmin ? `<div class="admin-section">
      <div class="admin-label">Gestion Admin</div>
      <div style="display:flex;gap:0.5rem;align-items:center">
        <input type="text" class="input-sm" id="map-url-input" placeholder="URL de l'image de la carte" value="${mapDoc?.imageUrl || ''}" style="max-width:400px">
        <button class="btn btn-gold btn-sm" onclick="saveMapUrl()">Enregistrer</button>
      </div>
    </div>` : ''}
    <div class="map-container">
      ${mapDoc?.imageUrl
        ? `<img src="${mapDoc.imageUrl}" class="map-img" alt="Carte">`
  ...
  async npcs() {
    ...
    const content = document.getElementById('main-content');
    let html = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">👥 PNJ Rencontrés</span></div>
    ...
    if (items.length === 0) {
      ...
      items.forEach((npc) => {
        const dispColor = npc.disposition === 'Amical' || npc.disposition === 'Allié'
          ? 'green'
          : npc.disposition === 'Hostile' || npc.disposition === 'Ennemi'
            ? 'red'
            : 'blue';
  ...
  async bastion() {
    const bastionDoc = await getDocData('bastion', 'main');
    const data = bastionDoc || getDefaultBastion();
    STATE._bastionData = data;
    const content = document.getElementById('main-content');
    content.innerHTML = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">🏰 Le Bastion</span></div>
      ...
      <div class="card">
        ...
        ${(data.salles || []).map((s) => `
          <div class="bastion-room">
            ...
            <div class="room-level">${[1,2,3].map((l) => `<span class="${l <= s.niveau ? 'filled' : ''}"></span>`).join('')}</div>
          </div>`).join('')}
  ...
  async story() {
    ...
    const content = document.getElementById('main-content');
    let html = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">📚 La Trame</span></div>
  ...
  async achievements() {
    ...
    const content = document.getElementById('main-content');
    let html = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">🏆 Hauts-Faits</span></div>
  ...
  async tutorial() {
    ...
    const content = document.getElementById('main-content');
    content.innerHTML = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">📕 Tutoriel de Jeu</span></div>
  ...
  async admin() {
    ...
    const content = document.getElementById('main-content');
    content.innerHTML = `<div class="page-header">
      <div class="page-title"><span class="page-title-accent">⚙️ Panneau Admin</span></div>
    ...
    <div class="grid-2">
      <div class="card">
        ...
        <table class="data-table">
          ...
          <tbody>
            ${users.filter((u) => u.email !== ADMIN_EMAIL).map((u) => `<tr>
              <td>${u.pseudo || '-'}</td>
  ...
  async informations() {
    const infoDoc = await getDocData('informations', 'main');
    const content = document.getElementById('main-content');
    ...
    const sections = infoDoc?.sections || defaultSections;
  ...
  async recettes() {
    const recettesDoc = await getDocData('recettes', 'main');
    const content = document.getElementById('main-content');
    const recettes = recettesDoc?.recettes || [];
    const potions = recettesDoc?.potions || [];
    ...
    <div class="grid-2" style="gap:1.5rem">
      <div>
        ...
        ${recettes.length === 0 ? `<div class="empty-state"><span class="icon">🍳</span><p>Aucune recette de cuisine.</p></div>` : recettes.map((r) => `<div class="card" style="margin-bottom:0.8rem;padding:1rem">
      ...
      <div>
        ...
        ${potions.length === 0 ? `<div class="empty-state"><span class="icon">🧪</span><p>Aucune potion.</p></div>` : potions.map((p) => `<div class="card" style="margin-bottom:0.8rem;padding:1rem">
          <div style="display:flex;justify-content:space-between">
            <div>
