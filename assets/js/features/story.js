...
function openStoryModal(item) {
  openModal(item?'✏️ Modifier':'📚 Nouvel Événement / Mission', `
    ...
    <script>document.getElementById('st-type').addEventListener('change',e=>{document.getElementById('st-mission-extra').style.display=e.target.value==='mission'?'':'none';});<\/script>
  `);
}
...
async function saveStory(id) {
  ...
  const data = {
    type, date: document.getElementById('st-date')?.value||'',
  ...
  closeModal(); showNotif('Événement enregistré !','success'); PAGES.story();
}
...
async function deleteStory(id) {
  ...
  await deleteFromCol('story',id); showNotif('Supprimé.','success'); PAGES.story();
}
