  ...
  return `<div class="qv-block">
    ...
    <div class="qv-spells">
      ${actives.map(s => {
        ...
        return `<span class="qv-spell${s.alwaysPrepared ? ' is-always' : ''}" title="${_esc(vm.effet)}">${_esc(vm.icon)} ${_esc(s.nom || '?')}${s.alwaysPrepared ? '<span class="qv-spell-free" title="Toujours prêt · hors capacité">∞</span>' : ''}${vm.pm != null && vm.resource !== 'none' ? ` <span class="qv-spell-cost">${vm.pm}${_esc(vm.resLabel)}</span>` : ''}</span>`;
      }).join('')}
    </div>
  </div>`;
}
